...
from haystack.nodes import (BM25Retriever, MarkdownConverter, PromptNode,
                            PromptTemplate)
    ...
    for directory_path in markdown_files_directory_paths:
        ...
        if directory.exists() and directory.is_dir():
            for markdown_file_path in directory.rglob("*.md"):
                ...
                if isinstance(docs, dict):  # Convert single doc to list
                    docs = [docs]
...
Question: {query}"""
    ...
    print_answer = lambda out: print(out["results"][0].strip())
